...
export const DEFAULT_SETTINGS = {
  ...
  income_categories: JSON.stringify(['จ่ายฟรี/พักงวด', 'ค่าธรรมเนียมอื่น', 'รายรับอื่น']),
};

/**
 * หมวดรายรับของเงิน "จ่ายฟรี/พักงวด"
 *
 * ลูกหนี้รายวันที่ส่งงวดปกติไม่ไหว จ่ายเงินจำนวนหนึ่งแทนเพื่อพักงวดวันนั้น
 * เงินก้อนนี้ไม่ตัดยอดสัญญา ไม่ตัดเงินต้น ไม่ลดจำนวนงวด และไม่สร้างยอดค้าง
 *
 * เก็บใน income_entries ไม่ใช่ payments โดยตั้งใจ เพราะ
 *   1) ตาราง payments บังคับว่า ดอก + ต้น = ยอดที่จ่าย ซึ่งรายการนี้เป็น 0 + 0 ≠ ยอด
 *   2) ถ้าเก็บใน payments จะไปลด "ยอดค้าง" และเพิ่ม "ยอดเก็บได้วันนี้" ผิดทันที
 *      เพราะสองตัวเลขนั้นรวมมาจาก payments โดยตรง
 * ตรงกับที่ผู้ใช้ระบุว่าต้องแยกเป็นรายรับต่างหากจากดอกเบี้ยตามสัญญา
 */
export const FREE_PAY_CATEGORY = 'จ่ายฟรี/พักงวด';

/**
 * หมวดรายได้ "ดอกเบี้ยรับรู้ตอนรียอด"
 *
 * ตอนรียอด ยอดที่ยกไปสัญญาใหม่มีดอกเบี้ยเดิมที่ยังไม่ได้รับรู้ปนอยู่
 * ถ้าปล่อยให้ก้อนนั้นกลายเป็น "เงินต้น" ของสัญญาใหม่เฉย ๆ
 * มันจะไม่ถูกนับเป็นรายได้อีกเลยตลอดกาล เพราะเงินต้นรับคืนไม่ใช่รายได้
 * กำไรที่รายงานจะขาดหายไปทุกครั้งที่มีการรียอด
 *
 * จึงรับรู้เป็นรายได้ ณ วันที่รียอด ซึ่งตรงกับที่เจ้าของกิจการระบุว่า
 * "ได้ดอกเบี้ยก็ต่อเมื่อสิ้นสุดสัญญาหรือรียอด"
 */
export const REYOD_INTEREST_CATEGORY = 'ดอกเบี้ยรับรู้ตอนรียอด';

/**
 * คู่กันกับข้างบน — สัญญาเหมารวม (flat_total) ตามสเปกข้อ 14:
 * "ดอกเบี้ยตามสัญญาจะรับรู้เมื่อ ลูกค้าปิดยอด หรือ ลูกค้ารียอด เท่านั้น"
 * ระหว่างสัญญา เงินที่ลูกหนี้ส่งเป็นแค่ "รับชำระตามสัญญา" ยังไม่เป็นรายได้ดอกเบี้ย
 * เมื่อสัญญาปิดครบ ระบบบันทึกดอกทั้งก้อนของสัญญาเป็นรายได้ด้วยหมวดนี้
 * (ไม่ใช่เงินสดเข้าใหม่ — เงินสดเข้ามาแล้วระหว่างทาง จึงถูกแยกออกจากกระแสเงินสด)
 */
export const CLOSE_INTEREST_CATEGORY = 'ดอกเบี้ยรับรู้ตอนปิดสัญญา';

/**
 * โหมด "หักดอกก่อน" (สเปกข้อ 9, 15): ดอกทั้งก้อนถูกหักออกจากเงินที่จ่ายให้ลูกค้า
 * ณ วันเปิดสัญญา จึงรับรู้เป็นรายได้ทันทีวันนั้น — ต่างจากสองหมวดข้างบน
 * หมวดนี้เป็น "เงินสด" จริง (ทำให้เงินสดออกน้อยลง) ต้องนับในกระแสเงินสดด้วย
 */
export const UPFRONT_INTEREST_CATEGORY = 'ดอกเบี้ยหักก่อน (รับรู้ตอนเปิดสัญญา)';

/**
 * ขารับคู่ของการรียอดแบบ "ฐานเต็มยอด" (reyod_cash_basis = 'full')
 *
 * ฐานเต็มยอดบันทึกรายจ่ายเงินปล่อยเท่ายอดสัญญาใหม่ทั้งก้อน (ยอดยก + เงินเพิ่ม)
 * ในความเป็นจริงยอดยกไม่ใช่เงินสดที่จ่ายออกไปจริง — เสมือนนับเงินเต็มก้อนแล้ว
 * รับยอดยกกลับมาปิดสัญญาเดิมทันที ถ้าไม่บันทึกขารับก้อนนี้ เงินสดในบัญชี
 * จะ "หาย" เท่ากับยอดยกทุกครั้งที่รียอด (ยอดปิดวันไม่ตรงกับเงินสดจริง)
 *
 * เป็นเงินต้นรับคืน ไม่ใช่รายได้ — อยู่ในกระแสเงินสดแต่ไม่เข้ากำไร
 */
export const REYOD_CARRY_RETURN_CATEGORY = 'เงินต้นรับคืนจากการรียอด (ฐานเต็มยอด)';

/**
 * ค่าทำสัญญาเป็นรายได้ของ "พนักงานผู้เปิดสัญญา" ไม่ใช่รายได้กิจการ (สเปกข้อ 21)
 *
 * เงินที่หักจากลูกค้าตอนเปิดสัญญา (หมวด 'doc_fee' ใน income_entries) คือเงินที่
 * กิจการ "ถือแทนพนักงาน" — เข้ากระแสเงินสดแต่ไม่เข้ากำไร จนกว่าจะจ่ายให้พนักงาน
 * ตอนจ่ายใช้หมวดรายจ่ายนี้: เงินสดออกจริง แต่ไม่ใช่ค่าใช้จ่ายดำเนินงาน
 * (เป็นการชำระหนี้สินค้างจ่ายพนักงาน กำไรกิจการไม่กระทบทั้งขาเข้าและขาออก)
 */
export const DOC_FEE_PAYOUT_CATEGORY = 'จ่ายค่าทำสัญญาให้พนักงาน';

/**
 * ถอนดอกเบี้ยออกมาใช้ (สเปกข้อ 33-40)
 *
 * เจ้าของนำ "ดอกเบี้ยที่รับรู้แล้ว" ออกมาใช้ — เงินสดออกจริง แต่ไม่ใช่
 * ค่าใช้จ่ายดำเนินงาน (ข้อ 33 ระบุชัดว่าห้ามปนกับค่าแรง/ค่าน้ำมัน/ค่าใช้จ่ายทั่วไป)
 * จึงไม่แตะกำไร — ลักษณะเดียวกับถอนเงินทุน แต่แยกหมวดเพื่อคุมเพดาน
 * "คงเหลือ = รับรู้สะสม − ถอนสะสม" และห้ามถอนเกิน (ข้อ 38)
 *
 * รายได้ดอกลอยเก็บแยกจากดอกตามสัญญา (ข้อ 39) จึงมีหมวดถอนแยกกันคนละหมวด
 */
export const WITHDRAW_INTEREST_CATEGORY = 'ถอนดอกเบี้ยออกใช้';
export const WITHDRAW_FLOATING_CATEGORY = 'ถอนรายได้ดอกลอยออกใช้';
