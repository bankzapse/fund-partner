...
export function toSatang(baht) {
  ...
  if (!Number.isFinite(n)) throw new Error('จำนวนเงินไม่ถูกต้อง');
...
export function assertNonNegative(satang, label = 'จำนวนเงิน') {
  if (!Number.isInteger(satang)) throw new Error(`${label} ไม่ถูกต้อง`);
  if (satang < 0) throw new Error(`${label} ต้องไม่ติดลบ`);
  return satang;
}

export function assertPositive(satang, label = 'จำนวนเงิน') {
  assertNonNegative(satang, label);
  if (satang === 0) throw new Error(`${label} ต้องมากกว่า 0`);
  return satang;
}
