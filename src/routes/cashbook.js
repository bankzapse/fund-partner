...
router.post(
  ...
  wrap(async (req, res) => {
    ...
    // รายการที่ระบบสร้างเองคู่กับสัญญา (มี contract_id) ห้ามยกเลิกเดี่ยว ๆ จากสมุดเงินสด
    // ครอบคลุม: เงินปล่อยใหม่ (เงินสดออก), ค่าทำเอกสาร, จ่ายฟรี/พักงวด, ดอกเบี้ยรับรู้ตอนรียอด
    // ถ้ายกเลิกเดี่ยวได้ ยอดจะไม่สมดุล — เช่นลบ "เงินปล่อยใหม่" ทำให้เงินสดสุทธิพองขึ้น
    // ทั้งที่ปล่อยเงินไปจริง หรือลบดอกเบี้ยรียอดทำให้กำไรหายทั้งที่หนี้ยังอยู่
    // ต้องแก้ที่ต้นทาง (ยกเลิกสัญญา/การรียอด/การจ่ายฟรี) ไม่ใช่ลบรายการบัญชีทิ้ง
    if (before.contract_id) {
      const label =
        before.category === REYOD_INTEREST_CATEGORY
          ? 'ระบบสร้างขึ้นจากการรียอด'
          : 'ระบบสร้างขึ้นอัตโนมัติคู่กับสัญญา';
      return res.status(400).json({
        error: `รายการนี้${label} ยกเลิกเดี่ยวจากสมุดเงินสดไม่ได้ ` +
               'เพราะจะทำให้ยอดเงินสดและกำไรไม่ตรงกับความจริง — ต้องแก้ที่ต้นทางของรายการแทน',
      });
    }
