...
import { before, beforeEach, describe, it as test } from 'node:test';
import assert from 'node:assert/strict';

import { db, get, run, all, getSetting, setSetting } from '../src/db/index.js';
...
function setup() {
  ...
  owner = get(`SELECT * FROM users WHERE username='owner'`);
  collector = get(`SELECT * FROM users WHERE username='collector'`);
  run(
    `INSERT OR IGNORE INTO employees (user_id, code, full_name, is_active, created_at, updated_at)
     VALUES (:uid, 'E001', 'พนักงานเก็บเงิน', 1, :now, :now)`,
    { uid: collector.id, now },
  );
  empId = get(`SELECT id FROM employees WHERE code='E001'`).id;
...
function newDebtor(name = 'ลูกหนี้ทดสอบ', employeeId = empId) {
  const now = nowISO();
  const info = run(
...
function standardContract({ deductFirst = true, startDate = today() } = {}) {
...
describe('ข้อ 18 — เกณฑ์ทดสอบรับมอบงาน', () => {
  test('สัญญา 1,000 ค่าทำเอกสาร 100 งวดแรก 50 → ลูกค้าได้รับจริง 850 และงวดแรกถูกบันทึกแล้ว', () => {
    const { contract, preview, firstPayment } = standardContract();
  ...
  test('รับชำระ 50 บาท → ดอก 20 ต้น 30 สถานะเต็มงวด', () => {
  ...
  test('รับชำระ 20 บาท → ดอก 20 ต้น 0 สถานะเฉพาะดอก และเงินต้นไม่เปลี่ยน', () => {
    const { contract } = standardContract();
    const before = get(`SELECT principal_remaining FROM contracts WHERE id = :id`, { id: contract.id });
    const p = recordPayment(
      { contractId: contract.id, amountPaid: 2_000, paidDate: addDays(today(), 1) },
      collectorCtx,
    );
    const after = get(`SELECT principal_remaining FROM contracts WHERE id = :id`, { id: contract.id });
  ...
  test('รับชำระ 30 บาท → ดอก 20 ต้น 10 สถานะบางส่วน', () => {
    const { contract } = standardContract();
    const p = recordPayment(
  ...
  test('ไม่ชำระ (0 บาท) → บันทึกสถานะค้างชำระได้ ต้น/ดอกเป็น 0', () => {
    const { contract } = standardContract();
    const p = recordPayment(
  ...
  test('รียอดจากต้นคงเหลือ 700 เพิ่มใหม่ 500 → สัญญาใหม่ 1,200 และสัญญาเดิมปิดด้วยการรียอด', () => {
    const { contract } = standardContract({ deductFirst: false });
    // ปรับเงินต้นคงเหลือให้เป็น 700 บาทตามโจทย์
    run(`UPDATE contracts SET principal_remaining = 70_000 WHERE id = :id`, { id: contract.id });
    ...
    const link = get(`SELECT * FROM contract_links WHERE from_contract_id = :id`, { id: contract.id });
    assert.equal(link.to_contract_id, result.new_contract.id);
    assert.equal(link.carried_principal, 70_000);
    assert.ok(get(`SELECT id FROM contracts WHERE id = :id`, { id: contract.id }), 'สัญญาเดิมยังอยู่');
  });

  test('รียอด: เงินสดที่ลูกค้าได้รับคำนวณจากเงินเพิ่มใหม่ ไม่จ่ายซ้ำในส่วนเงินต้นเดิม (ข้อ 9)', () => {
    const { contract } = standardContract({ deductFirst: false });
    run(`UPDATE contracts SET principal_remaining = 70_000 WHERE id = :id`, { id: contract.id });
    const result = reyod({ fromContractId: contract.id, newMoney: 50_000 }, ctx);
  ...
  test('ยกเลิกรายการรับเงิน → ยอดถูกย้อนกลับและมีประวัติผู้ยกเลิก', () => {
    const { contract } = standardContract();
    const before = contractSummary(contract.id);
    const p = recordPayment(
      { contractId: contract.id, amountPaid: 5_000, paidDate: addDays(today(), 1) },
      collectorCtx,
    );
    const mid = contractSummary(contract.id);
    assert.equal(mid.principal_remaining, before.principal_remaining - 3_000);

    const voided = voidPayment({ paymentId: p.id, reason: 'บันทึกผิดสัญญา' }, ctx);
    const after = contractSummary(contract.id);
    ...
    assert.ok(get(`SELECT id FROM payments WHERE id = :id`, { id: p.id }));
  ...
  test('ปิดยอดประจำวัน → แสดงยอดระบบ เงินสดจริง และส่วนต่าง', () => {
    const date = '2030-01-15';
    const debtorId = newDebtor('ลูกหนี้ปิดยอด');
    createContract(
      {
        debtorId,
        employeeId: empId,
    ...
    const preview = closingPreview(date);
    ...
    const closing = closeDay({ date, actualCash: -84_000, note: 'ทดสอบ' }, ctx);
    ...
    const c = get(`SELECT id FROM contracts WHERE start_date = :d`, { d: date });
    assert.throws(
      () => recordPayment({ contractId: c.id, amountPaid: 5_000, paidDate: date }, collectorCtx),
      /ปิดยอดประจำวันแล้ว/,
    );
  });

  test('พนักงานเก็บเงิน Login → เห็นเฉพาะลูกหนี้ที่ตนรับผิดชอบ', () => {
    ...
    createContract(
      {
        debtorId: newDebtor('ลูกหนี้ของพนักงาน E001'),
        employeeId: empId,
        type: 'daily24',
        principalAmount: 100_000,
        installmentAmount: 5_000,
    ...
    const mine = dueToday({ date: today(), employeeId: empId });
    const others = dueToday({ date: today(), employeeId: -1 });
  ...
  test('รายงานกำไร → ไม่นำเงินต้นรับคืนมารวมเป็นรายได้', () => {
    const date = '2030-03-10';
    const debtorId = newDebtor('ลูกหนี้รายงาน');
    ...
    recordPayment({ contractId: contract.id, amountPaid: 5_000, paidDate: payDate }, collectorCtx);

    const s = financeSummary({ from: payDate, to: payDate });
...
describe('ข้อ 3 — กติกาการคำนวณ', () => {
  test('ตัดดอกเบี้ยก่อนเสมอ แล้วจึงตัดเงินต้น', () => {
    const insts = [
  ...
  test('จ่ายเกินงวดปัจจุบันจะไหลไปงวดถัดไป', () => {
    const insts = [
      { id: 1, seq: 1, interest_due: 2000, principal_due: 3000, interest_paid: 0, principal_paid: 0 },
  ...
  test('ดอกลอย: จ่ายตามรอบ เงินต้นคงเดิม', () => {
    const debtorId = newDebtor('ลูกหนี้ดอกลอย');
    const { contract } = createContract(
      {
        debtorId,
        employeeId: empId,
  ...
  test('ดอกลอย: ชำระเกินดอกจะไปตัดเงินต้น', () => {
    const debtorId = newDebtor('ลูกหนี้ดอกลอย 2');
    const { contract } = createContract(
      {
        debtorId,
        type: 'floating',
  ...
  test('นับจำนวนงวดเต็มที่ชำระแล้วและจำนวนวันที่จ่ายเฉพาะดอกแยกกัน (ข้อ 3.1)', () => {
    const { contract } = standardContract({ startDate: '2031-01-01' });
    recordPayment({ contractId: contract.id, amountPaid: 5_000, paidDate: '2031-01-02' }, ctx); // เต็มงวด
    recordPayment({ contractId: contract.id, amountPaid: 2_000, paidDate: '2031-01-03' }, ctx); // เฉพาะดอก
    recordPayment({ contractId: contract.id, amountPaid: 1_000, paidDate: '2031-01-04' }, ctx); // บางส่วน

    const s = contractSummary(contract.id, '2031-01-04');
...
describe('ข้อ 14 — กฎธุรกิจและการตรวจสอบข้อมูล', () => {
  test('ห้ามรับยอดติดลบ', () => {
    const { contract } = standardContract();
    assert.throws(
      () => recordPayment({ contractId: contract.id, amountPaid: -100 }, ctx),
      /ต้องไม่ติดลบ/,
    );
  });

  test('เงินต้นคงเหลือต้องไม่ต่ำกว่า 0 — จ่ายเกินภาระหนี้ถูกปฏิเสธ', () => {
    const { contract } = standardContract();
    assert.throws(
      () => recordPayment({ contractId: contract.id, amountPaid: 10_000_000 }, ctx),
      /เกินภาระหนี้/,
    );
  });

  test('ยอดดอกเบี้ย + เงินต้น ต้องเท่ากับยอดรับจริงเสมอ', () => {
    const { contract } = standardContract();
    for (const amount of [1_000, 2_000, 3_000, 5_000, 7_500]) {
      const p = previewPayment({ contractId: contract.id, amountPaid: amount });
      assert.equal(p.interest_amount + p.principal_amount, amount);
    }
  });

  test('สัญญาที่ปิด/รียอดแล้วห้ามรับชำระเพิ่ม เว้นแต่เจ้าของอนุมัติ', () => {
    const { contract } = standardContract({ deductFirst: false });
    reyod({ fromContractId: contract.id, newMoney: 0 }, ctx);
    assert.throws(
      () => recordPayment({ contractId: contract.id, amountPaid: 5_000 }, collectorCtx),
      /ปิดหรือรียอดแล้ว/,
    );
    // เจ้าของอนุมัติได้
    const ok = recordPayment(
  ...
  test('เลขที่สัญญาและเลขที่ใบรับเงินต้องไม่ซ้ำ', () => {
    const nos = new Set();
    for (let i = 0; i < 5; i++) {
      const { contract } = standardContract({ startDate: '2032-02-02' });
      assert.equal(nos.has(contract.contract_no), false);
      nos.add(contract.contract_no);
    }
    const receipts = all(`SELECT receipt_no FROM payments`).map((r) => r.receipt_no);
    assert.equal(new Set(receipts).size, receipts.length);
  });

  test('ค่าทำเอกสารตั้งค่าได้จากหน้าตั้งค่า (ข้อ 7.2)', () => {
    setSetting('doc_fee', '25000', owner.id);
    const p = previewContract({
      debtorId: newDebtor(),
      type: 'daily24',
      principalAmount: 100_000,
      installmentAmount: 5_000,
    ...
    setSetting('doc_fee', '10000', owner.id);
  });
});
