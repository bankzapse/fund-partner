...
describe('บั๊กที่พบจากการตรวจแบบปฏิปักษ์ (กันย้อนกลับ)', () => {
  ...
  test('หักงวดแรกได้ไม่เกินเงินที่จ่ายออกจริง และต้องเตือน', async () => {
    // เดิมใช้ Math.max(0,...) กลืนส่วนติดลบทิ้ง แล้วยังบันทึกเงินเข้าเต็มจำนวน
    // ทำให้เงินสดในระบบงอกขึ้นเองทุกครั้ง
    const res = await api('owner', 'POST', '/api/contracts/preview', {
    ...
    assert.match(p.warnings.join(' '), /หักงวดแรกได้/, 'ต้องบอกว่าหักได้เท่าไร');
    assert.ok(p.first_installment <= p.gross_out, 'หักได้ไม่เกินเงินที่จ่ายออก');
    assert.equal(p.doc_fee + p.first_installment + p.cash_to_customer, p.gross_out,
      'ผลรวมต้องเท่าเงินที่จ่ายออกพอดี ไม่มีส่วนเกินหล่นหาย');
  });
...
describe('เฟส 2 — รอบตรวจที่สอง', () => {
  test('รียอดไม่เติมเงิน ต้องไม่ทำให้เงินสดในระบบงอกขึ้นเอง', async () => {
    // ค่าตั้งต้นของระบบคือค่าเอกสาร 100 + หักงวดแรก ซึ่งเป็นสิ่งที่หน้าจอส่งจริง
    // เดิมบันทึกทั้งสองก้อนเป็นเงินเข้าทั้งที่ไม่มีเงินจ่ายออก = งอก 170 บาทต่อครั้ง
    const { financeSummary } = await import('../src/domain/reports.js');
    const c = await makeFlatContract({ principal: 200000, rateBp: 2000, n: 24 });
    const inst = await all(
      `SELECT * FROM installments WHERE contract_id = :c ORDER BY seq LIMIT 10`, { c: c.id },
    ...
    const before = await financeSummary({ from: d, to: d });
    const res = await api('owner', 'POST', '/api/contracts/reyod', {
      from_contract_id: c.id, new_money: 0, doc_fee: 10000, start_date: d,
    });
    assert.equal(res.status, 201, JSON.stringify(res.body));
    const after = await financeSummary({ from: d, to: d });
    assert.equal(after.total_in, before.total_in, 'เงินสดเข้าต้องไม่งอก');
    assert.equal(after.net_cash, before.net_cash, 'เงินสดสุทธิต้องไม่เปลี่ยน');
  });

  test('รียอดต้องติดด่านปิดยอดประจำวัน', async () => {
    const c = await makeFlatContract({ principal: 200000, rateBp: 2000, n: 24 });
    const inst = await all(
      `SELECT * FROM installments WHERE contract_id = :c ORDER BY seq LIMIT 3`, { c: c.id },
    );
    for (const r of inst) {
      await api('owner', 'POST', '/api/payments', {
        contract_id: c.id, amount_paid: r.due_amount, paid_date: r.due_date,
    ...
    await api('owner', 'POST', '/api/cashbook/closing', { closing_date: d, actual_cash: 0 });
    const res = await api('owner', 'POST', '/api/contracts/reyod', {
      from_contract_id: c.id, new_money: 0, doc_fee: 0, deduct_first: false, start_date: d,
    });
    assert.equal(res.status, 400, 'วันที่ปิดยอดแล้วต้องรียอดไม่ได้');
    assert.match(res.body.error, /ปิดยอดประจำวันแล้ว/);
    await run(`DELETE FROM daily_closings WHERE closing_date = :d`, { d });
  });

  test('สัญญาที่ปิดด้วยการรียอดแล้ว ยอดคงเหลือต้องเป็น 0', async () => {
    const c = await makeFlatContract({ principal: 200000, rateBp: 2000, n: 24 });
    const inst = await all(
      `SELECT * FROM installments WHERE contract_id = :c ORDER BY seq LIMIT 5`, { c: c.id },
    );
    ...
    await api('owner', 'POST', '/api/contracts/reyod', {
      from_contract_id: c.id, new_money: 0, doc_fee: 0, deduct_first: false,
    });
    const list = await api('owner', 'GET', '/api/contracts?limit=100');
    const closed = list.body.items.find((x) => x.id === c.id);
    assert.equal(closed.status, 'closed_reyod');
    assert.equal(closed.outstanding, 0, 'สัญญาที่ปิดแล้วต้องไม่ค้างยอด');
  });
});

describe('เฟส 2 — รายงานประวัติรียอด', () => {
  test('เงินต้นที่ยกไป + ดอกเดิมที่ยกไป + เงินเพิ่มใหม่ = ยอดสัญญาใหม่', async () => {
    // ถ้าไม่แสดงดอกที่ยกไป คอลัมน์จะบวกกันไม่ลงตัว
    // ผู้อ่านจะเห็นยอดสัญญาใหม่โตขึ้นเองโดยไม่มีที่มา
    const c = await makeFlatContract({ principal: 200000, rateBp: 2000, n: 24 });
    const inst = await all(
      `SELECT * FROM installments WHERE contract_id = :c ORDER BY seq LIMIT 10`, { c: c.id },
    );
    ...
    await api('owner', 'POST', '/api/contracts/reyod', {
      from_contract_id: c.id, new_money: 60000, doc_fee: 0, deduct_first: false,
    });
    const res = await api('owner', 'GET', '/api/reports/reyod');
    assert.equal(res.status, 200);
    const row = res.body.items.find((x) => x.from_contract_id === c.id);
    assert.ok(row, 'ต้องมีรายการในรายงาน');
    assert.equal(
