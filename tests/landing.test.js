...
describe('ราคาบนหน้าแนะนำระบบ', () => {
  ...
  test('เปิดไฟล์ต้นแบบตรง ๆ ไม่ได้ ต้องเด้งกลับหน้าแรก', async () => {
    // landing.html คือต้นแบบที่ยังไม่ได้เติมราคา ถ้าเปิดได้ Google อาจเก็บไปเป็น
    // เนื้อหาซ้ำที่ไม่มีราคา และคนที่หลงเข้ามาจะเห็นหน้าที่ขาดส่วนสำคัญ
    const res = await fetch(base + '/landing.html', { redirect: 'manual' });
    assert.equal(res.status, 301, 'ต้องส่งต่อถาวรไปหน้าแรก');
